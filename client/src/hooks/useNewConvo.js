...
import { updateLastSelectedModel, getLocalStorageItems, getDefaultModelSpec, getDefaultEndpoint, getModelSpecPreset, buildDefaultConvo, logger, } from '~/utils';
...
const useNewConvo = (index = 0) => {
    ...
    const switchToConversation = useRecoilCallback(() => async (conversation, preset = null, modelsData, buildDefault, keepLatestMessage, keepAddedConvos, disableFocus, _disableParams) => {
    ...
    const newConversation = useCallback(function createNewConvo({ template: _template = {}, preset: _preset, modelsData, disableFocus, buildDefault = true, keepLatestMessage = false, keepAddedConvos = false, disableParams, } = {}) {
        pauseGlobalAudio();
        if (!saveBadgesState) {
            resetBadges();
        }
        ...
        switchToConversation(conversation, preset, modelsData, buildDefault, keepLatestMessage, keepAddedConvos, disableFocus, disableParams);
    }, [
        files,
        setFiles,
        saveDrafts,
