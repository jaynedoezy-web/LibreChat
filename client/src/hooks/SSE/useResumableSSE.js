...
import { request, Constants, QueryKeys, ErrorTypes, apiBaseUrl, createPayload, ViolationTypes, LocalStorageKeys, removeNullishValues, } from 'librechat-data-provider';
...
export default function useResumableSSE(submission, chatHelpers, isAddedRequest = false, runIndex = 0) {
    ...
    const { setMessages, getMessages, setConversation, setIsSubmitting, newConversation, resetLatestMessage, } = chatHelpers;
    const { stepHandler, finalHandler, errorHandler, clearStepMaps, messageHandler, contentHandler, createdHandler, syncStepMessage, attachmentHandler, resetContentHandler, } = useEventHandlers({
        setMessages,
        getMessages,
        setCompleted,
        isAddedRequest,
        ...
        sse.addEventListener('message', (e) => {
            try {
                ...
                if (data.created != null) {
                    ...
                    createdHandler(data, { ...currentSubmission, userMessage });
                    return;
                }
     ...
     */
    const startGeneration = useCallback(async (currentSubmission) => {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (isNetworkError && attempt < maxRetries) {
                    ...
                    console.log(`[ResumableSSE] Network error starting generation, retrying in ${delay}ms (attempt ${attempt}/${maxRetries})`);
