...
import { logger, setDraft, scrollToEnd, getAllContentText, addConvoToAllQueries, updateConvoInAllQueries, removeConvoFromAllQueries, findConversationInInfinite, } from '~/utils';
...
export default function useEventHandlers({ setMessages, getMessages, setCompleted, isAddedRequest = false, setConversation, setIsSubmitting, newConversation, setShowStopButton, resetLatestMessage, }) {
    ...
    const createdHandler = useCallback((data, submission) => {
        ...
        if (conversationId) {
            applyAgentTemplate({
                ...
                startupConfig: queryClient.getQueryData([QueryKeys.startupConfig]),
            });
        }
        if (resetLatestMessage) {
    ...
    const finalHandler = useCallback((data, submission) => {
        ...
        try {
            ...
            if (data.earlyAbort) {
                console.log('[finalHandler] Early abort detected - no messages saved, staying on new chat');
            ...
            if (setConversation && isAddedRequest !== true) {
                setConversation((prevState) => {
                    ...
                    const cachedConvo = queryClient.getQueryData([
                        QueryKeys.conversation,
