...
export default function WebSearch({ initialProgress: progress = 0.1, isSubmitting, isLast, output, }) {
    ...
    const progressText = useMemo(() => {
        let text = turns > 1 ? 'com_ui_web_searching_again' : 'com_ui_web_searching';
        if (showSources) {
    ...
    return (<>
      <div className="relative my-2.5 flex size-5 shrink-0 items-center gap-2.5">
        {showSources && (<div className="mr-2">
            <StackedFavicons sources={processedSources} start={-5}/>
          </div>)}
        <ProgressText finishedText="" hasInput={false} error={cancelled} isExpanded={false} progress={clampedProgress} inProgressText={progressText}/>
      </div>
    </>);
}
