...
import { Button, Spinner, OGDialog, Clipboard, CheckMark, OGDialogClose, OGDialogTitle, OGDialogContent, } from '@librechat/client';
...
const Mermaid = memo(({ children, id, theme }) => {
    ...
    const { processedSvg, parsedDimensions } = useMemo(() => {
        ...
        const applyFallbackFixes = (svgString) => {
    ...
    const handleDialogWheel = useCallback((e) => {
    ...
    const zoomControls = (<div className={cn('absolute bottom-2 right-2 z-10 flex items-center gap-1 rounded-md border border-border-light bg-surface-secondary p-1 shadow-lg transition-opacity duration-200', showControls ? 'opacity-100' : 'pointer-events-none opacity-0')}>
      <button type="button" onClick={(e) => {
            e.stopPropagation();
            handleZoomOut();
        }} disabled={zoom <= MIN_ZOOM} className="rounded p-1.5 text-text-secondary hover:bg-surface-hover disabled:opacity-40 disabled:hover:bg-transparent" title={localize('com_ui_zoom_out')}>
      ...
      <button type="button" onClick={(e) => {
            e.stopPropagation();
            handleZoomIn();
        }} disabled={zoom >= MAX_ZOOM} className="rounded p-1.5 text-text-secondary hover:bg-surface-hover disabled:opacity-40 disabled:hover:bg-transparent" title={localize('com_ui_zoom_in')}>
      ...
      <button type="button" onClick={(e) => {
            e.stopPropagation();
            handleResetZoom();
        }} disabled={zoom === 1 && pan.x === 0 && pan.y === 0} className="rounded p-1.5 text-text-secondary hover:bg-surface-hover disabled:opacity-40 disabled:hover:bg-transparent" title={localize('com_ui_reset_zoom')}>
        <RotateCcw className="h-4 w-4"/>
      </button>
      <div className="mx-1 h-4 w-px bg-border-medium"/>
      <button ref={zoomCopyButtonRef} type="button" onClick={(e) => {
            e.stopPropagation();
            handleZoomCopy();
        }} className="rounded p-1.5 text-text-secondary hover:bg-surface-hover" title={localize('com_ui_copy_code')}>
        {isZoomCopied ? <CheckMark className="h-4 w-4"/> : <Clipboard className="h-4 w-4"/>}
      </button>
    </div>);
    // Dialog zoom controls
    const dialogZoomControls = (<div className="absolute bottom-4 right-4 z-10 flex items-center gap-1 rounded-md border border-border-light bg-surface-secondary p-1 shadow-lg">
      <button type="button" onClick={(e) => {
            e.stopPropagation();
            handleDialogZoomOut();
        }} disabled={dialogZoom <= MIN_ZOOM} className="rounded p-1.5 text-text-secondary hover:bg-surface-hover disabled:opacity-40 disabled:hover:bg-transparent" title={localize('com_ui_zoom_out')}>
        <ZoomOut className="h-4 w-4"/>
      </button>
      <span className="min-w-[3rem] text-center text-xs text-text-secondary">
      ...
      <button type="button" onClick={(e) => {
            e.stopPropagation();
            handleDialogZoomIn();
        }} disabled={dialogZoom >= MAX_ZOOM} className="rounded p-1.5 text-text-secondary hover:bg-surface-hover disabled:opacity-40 disabled:hover:bg-transparent" title={localize('com_ui_zoom_in')}>
        <ZoomIn className="h-4 w-4"/>
      </button>
      <div className="mx-1 h-4 w-px bg-border-medium"/>
      <button type="button" onClick={(e) => {
        ...
        }} disabled={dialogZoom === 1 && dialogPan.x === 0 && dialogPan.y === 0} className="rounded p-1.5 text-text-secondary hover:bg-surface-hover disabled:opacity-40 disabled:hover:bg-transparent" title={localize('com_ui_reset_zoom')}>
        <RotateCcw className="h-4 w-4"/>
      </button>
      <div className="mx-1 h-4 w-px bg-border-medium"/>
      <button ref={dialogZoomCopyButtonRef} type="button" onClick={(e) => {
            e.stopPropagation();
    ...
    const expandedDialog = (<OGDialog open={isDialogOpen} onOpenChange={setIsDialogOpen} triggerRef={expandButtonRef}>
      <OGDialogContent showCloseButton={false} className="h-[85vh] max-h-[85vh] w-[90vw] max-w-[90vw] gap-0 overflow-hidden border-border-light bg-surface-primary-alt p-0">
        <OGDialogTitle className="flex h-10 items-center justify-between bg-gray-700 px-4 font-sans text-xs text-gray-200">
          ...
          <div className="flex gap-2">
            <Button ref={dialogShowCodeButtonRef} variant="ghost" size="sm" className="h-auto min-w-[6rem] gap-1 rounded-sm px-1 py-0 text-xs text-gray-200 hover:bg-gray-600 hover:text-white focus-visible:ring-white focus-visible:ring-offset-0" onClick={handleToggleDialogCode}>
              {dialogShowCode ? (<ChevronUp className="h-4 w-4"/>) : (<ChevronDown className="h-4 w-4"/>)}
              {dialogShowCode ? localize('com_ui_hide_code') : localize('com_ui_show_code')}
            </Button>
            <Button ref={dialogCopyButtonRef} variant="ghost" size="sm" className="h-auto gap-1 rounded-sm px-1 py-0 text-xs text-gray-200 hover:bg-gray-600 hover:text-white focus-visible:ring-white focus-visible:ring-offset-0" onClick={handleDialogCopy}>
        ...
        {dialogShowCode && (<div className="border-b border-border-medium bg-surface-secondary p-4">
            <pre className="max-h-[150px] overflow-auto whitespace-pre-wrap text-xs text-text-secondary">
              {children}
            </pre>
          </div>)}
        <div className={cn('relative flex-1 overflow-hidden p-4', 'bg-surface-primary-alt', isDialogPanning ? 'cursor-grabbing' : 'cursor-grab')} style={{ height: dialogShowCode ? 'calc(85vh - 200px)' : 'calc(85vh - 50px)' }} onWheel={handleDialogWheel} onMouseDown={handleDialogMouseDown} onMouseMove={handleDialogMouseMove} onMouseUp={handleDialogMouseUp} onMouseLeave={handleDialogMouseLeave}>
    ...
    if (isLoading) {
        ...
        if (lastValidSvgRef.current && blobUrl) {
            return (<div className={cn('relative w-full overflow-hidden rounded-md border transition-all duration-200', showControls ? 'border-border-light' : 'border-transparent')} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} onFocus={() => setIsFocusWithin(true)} onBlur={(e) => {
          ...
          <div ref={containerRef} className={cn('relative overflow-hidden p-4 transition-colors duration-200', 'rounded-md', showControls ? 'bg-surface-primary-alt' : 'bg-transparent', isPanning ? 'cursor-grabbing' : 'cursor-grab')} style={{ height: `${calculatedHeight}px` }} onMouseDown={handleMouseDown}>
            ...
            <div className="absolute inset-0 flex items-center justify-center" style={{
              ...
              <img src={blobUrl} alt="Mermaid diagram" className="select-none opacity-70" style={{
        ...
        <pre ref={streamingCodeRef} className="max-h-[350px] min-h-[150px] overflow-auto whitespace-pre-wrap rounded-b-md bg-surface-primary-alt p-4 font-mono text-xs text-text-secondary">
          {children}
        </pre>
      </div>);
    }
    ...
    if (error) {
        ...
        <div className="rounded-b-md border-t border-red-500/30 bg-red-500/10 p-4">
          ...
          {showCode && (<div className="mt-4 border-t border-border-medium pt-4">
    ...
    return (<>
      {expandedDialog}
      <div className={cn('relative w-full overflow-hidden rounded-md border transition-all duration-200', showControls ? 'border-border-light' : 'border-transparent')} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} onFocus={() => setIsFocusWithin(true)} onBlur={(e) => {
            if (!e.currentTarget.contains(e.relatedTarget)) {
                setIsFocusWithin(false);
            }
        }} onClick={handleContainerClick}>
        <MermaidHeader className={cn('absolute left-0 right-0 top-0 z-20', showControls ? 'opacity-100' : 'pointer-events-none opacity-0')} codeContent={children} showCode={showCode} showExpandButton expandButtonRef={expandButtonRef} onExpand={handleExpand} onToggleCode={handleToggleCode}/>
        {showCode && (<div className={cn('border-b border-border-medium bg-surface-secondary p-4 pt-12 transition-opacity duration-200', showControls ? 'opacity-100' : 'pointer-events-none opacity-0')}>
            <pre className="overflow-auto whitespace-pre-wrap text-xs text-text-secondary">
              {children}
            </pre>
          </div>)}
        <div ref={containerRef} className={cn('relative overflow-hidden p-4 transition-colors duration-200', 'rounded-md', showControls ? 'bg-surface-primary-alt' : 'bg-transparent', isPanning ? 'cursor-grabbing' : 'cursor-grab')} style={{ height: `${calculatedHeight}px` }} onMouseDown={handleMouseDown}>
          <div className="absolute inset-0 flex items-center justify-center" style={{
            ...
            <img src={blobUrl} alt="Mermaid diagram" className="select-none" style={{
            width: svgDimensions ? `${svgDimensions.width * initialScale}px` : 'auto',
            height: svgDimensions ? `${svgDimensions.height * initialScale}px` : 'auto',
        }} draggable={false}/>
          </div>
